...
Das Gebiet zwischen zwei Elementverbindungen wird wie in der Abbildung~\ref{fem:nd:abb}~(\subref{fem:nd:abb:basisfkt}) ``ausgefüllt''.
...
Es sollte ein Bild ähnlich des zweidimensionalen Beispiels in Abbildung~\ref{fem:nd:abb}~(\subref{fem:nd:abb:2d_mesh})
resultieren.
...
Es werden auch hier nur die Einträge ungleich Null sein, deren Elemente sich wie in Abbildung~\ref{fem:nd:abb}~(\subref{fem:nd:abb:basisfkt}) gezeigt berühren.
Der Eintrag in der Zeile $a$ und Spalte $b$ ist also nur dann ungleich Null, wenn Element $a$ und Element $b$ im Graphen, welcher in Abbildung~\ref{fem:nd:abb}~(\subref{fem:nd:abb:2d_mesh}) gezeigt ist, verbunden sind.
