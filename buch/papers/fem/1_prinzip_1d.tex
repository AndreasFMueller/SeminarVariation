...
Der lineare Ansatz verwendet die in Abbildung~\ref{fem:1d:abb:linear}~(\subref{fem:1d:abb:linear:formfkt}) gezeigten linearen Funktionen
\begin{equation}
    a_n(x) = \left\{ 
...
Die in Abbildung~\ref{fem:1d:abb:quadratisch}~(\subref{fem:1d:abb:quadratisch:formfkt}) gezeigten Formfunktionen lauten
\begin{align}
    a_n(x) &= \left\{ 
    \def\arraystretch{\formfktStretch}
    \begin{array}{ccc}
...
Die durch diese Funktionen interpolierte Funktion $f_\text{kubisch} (x)$ ist wie in Abbildung~\ref{fem:1d:abb:kubisch}~(\subref{fem:1d:abb:kubisch:vergleich}) zu sehen ist kaum mehr von der ursprünglichen Funktion $f(x)$ zu unterscheiden.
