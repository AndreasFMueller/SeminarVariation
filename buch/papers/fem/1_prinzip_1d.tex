...
\input{papers/fem/fig/quadratischer_ansatz.tex} % TODO: Widerholende Formfunktionen und Summe deren als Graphen zeigen -> Element kennzeichnen
Für das Interpolieren mit dem quadratischen Ansatz muss das Vorgehen leicht angepasst werden, da sonst die Bedingung \ref{fem:1d:bedindung_formfkt} nicht erfüllt wird. 
...
Dieser wird meistens in der Mitte des Elements gewählt.
...
\begin{equation}
    ...
    \end{array} \right.
\end{equation}
für die Knoten in der Mitte der Elemente hergeleitet werden.
...
\begin{equation}
    a_n(x_n) = 1, a_n'(x_n) = 0
    \quad \text{und} \quad 
    a_n(x_{n-1}) = a_n(x_{n+1}) = 0
\end{equation}
für Knotenpunkte in der Mitte der Elemente.
...
Da $\mathbf{L}$ die Gram-Matrix\footnote{TODO: cite correctly} des Skalarprodukts darstellt und das Skalarprodukt positiv definit ist, ist auch $\mathbf{L}$ positiv definit.
