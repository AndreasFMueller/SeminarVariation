...
\begin{tikzpicture}[>=latex,thick,scale=\skala]
    ...
    \draw[color=darkgrey]
        ...
        % Formfunktionen Elementenden
    \foreach \n/\col in {2/darkblue, 4/darkorange}{
        \draw[color=\col,line width=1.4pt] plot[domain=0:\n-2, scale=2, smooth]
        ({\x},{0});
        \draw[color=\col,line width=1.4pt] plot[domain=\n-2:\n, scale=2, smooth]
        ({\x},{\bspFkt(\n)/2*(\x-(\n-2))*(\x-(\n-1))});
    }
    \foreach \n/\col in {0/darkred, 2/darkblue}{
        \draw[color=\col,line width=1.4pt] plot[domain=\n:\n+2, scale=2, smooth]
        ({\x},{\bspFkt(\n)/2*(\x-(\n+2))*(\x-(\n+1))});
        \draw[color=\col,line width=1.4pt] plot[domain=\n+2:4, scale=2, smooth]
        ({\x},{0});
    }
        % Formfunktionen Elementmitten
    \foreach \n/\col in {1/darkpurple, 3/darkgreen}{
        \draw[color=\col,line width=1.4pt] plot[domain=0:\n-1, scale=2, smooth]
        ({\x},{0});
        \draw[color=\col,line width=1.4pt] plot[domain=\n-1:\n+1, scale=2, smooth]
        ({\x},{(-\bspFkt(\n)*(\x-\n+1)*(\x-\n-1))});
    ...
    % Elemente
    \draw [decorate,decoration={brace,amplitude=8pt,mirror,raise=3ex}]
        (0,0) -- (4,0) node[midway,yshift=-3em]{Element};
    \draw [decorate,decoration={brace,amplitude=8pt,mirror,raise=3ex}]
        (4,0) -- (8,0) node[midway,yshift=-3em]{Element};
    ...
    % y-Achse
    \draw[->] (0,{-0.1}) -- (0,{4.5})
    coordinate[label={right:$y$\vphantom{$(y)$}}];
    \node at (0,0) [left] {$0$};
    \draw (-0.1,4) -- (0.1,4);
    \node at (0,4) [left] {$3$};
    ...
    \foreach \x/\col in {0/darkred, 2/darkblue, 4/darkorange}{
        \node at (\x*2, {2*\bspFkt(\x)}) [yshift = 0.3cm, color=\col, inner sep=1pt, fill=white, draw, rounded rectangle, stroke=none] {${a_\x(x)}$};
    }
    \foreach \x/\col in {1/darkpurple, 3/darkgreen}{
        \node at (\x*2, {2*\bspFkt(\x)}) [yshift = 0.3cm, color=\col, inner sep=1pt, fill=white, draw, rounded rectangle, stroke=none] {${b_\x(x)}$};
    }
\end{tikzpicture}
