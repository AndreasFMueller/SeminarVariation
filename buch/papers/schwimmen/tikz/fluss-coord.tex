%
% fluss-coord.tex -- template for standalon tikz images
%
...
\definecolor{quer}{rgb}{0.961,0.651,0.137}
\begin{document}

    \tikzset{every picture/.style={line width=0.75pt}} %set default line width to 0.75pt        
    
    \begin{tikzpicture}[x=0.75pt,y=0.75pt,yscale=-1,xscale=1,>=latex]
    ...
    \draw[->] (268,223.4) -- ++(35,0) coordinate[label={right:$y$}];
    \draw[->] (270.94,226) -- ++(0,-35) coordinate[label={above:$x$}];
    ...
    % Text Node
    %\draw (415,84) node [anchor=north west][inner sep=0.75pt]  [color={rgb, 255:red, 245; green, 166; blue, 35 }  ,opacity=1 ] [align=left] {$B$};
    % Text Node
    %\draw (415,185) node [anchor=north west][inner sep=0.75pt]  [color={rgb, 255:red, 245; green, 166; blue, 35 }  ,opacity=1 ] [align=left] {$A$};
    % Text Node
    \draw (552,120) node [anchor=north west][inner sep=0.75pt]  [color={rgb, 255:red, 74; green, 144; blue, 226 }  ,opacity=1 ] [align=left] {flow};
    \draw (552,147) node [anchor=north west][inner sep=0.75pt]  [color={rgb, 255:red, 74; green, 144; blue, 226 }  ,opacity=1 ] [align=left] {$u(x)$};

    % Text Node
    %\draw (267,182) node [anchor=north west][inner sep=0.75pt]  [color={rgb, 255:red, 0; green, 0; blue, 0 }  ,opacity=1 ] [align=left] {{$x$}};
    %% Text Node
    %\draw (301,213) node [anchor=north west][inner sep=0.75pt]  [color={rgb, 255:red, 0; green, 0; blue, 0 }  ,opacity=1 ] [align=left] {{$y$}};

    \coordinate (A) at (410,176);
    \coordinate (C) at (385,144);
    \coordinate (B) at (410,106);
    
    \fill[color=quer] (A) circle[radius=3.9];
    \fill[color=quer] (B) circle[radius=3.9];
    \draw[->,color=quer,line width=1.5pt,shorten >= 2] (A) to[out=-150,in=90] (C) to[out=-90,in=135] (B);

    \node[color=quer] at ($(A)+(0,4)$) [below] {$A$};
    \node[color=quer] at ($(B)+(0,-4)$) [above] {$B$};
